...
#define ngx_spdy_frame_parse_delta_window_size(p)                             \
    (ngx_spdy_frame_parse_uint32(p) & 0x7fffffff)

#define ngx_spdy_ctl_frame_check(h)                                           \
    (((h) & 0xffffff00) == ngx_spdy_v3_ctl_frame_head(0))
...
#define ngx_spdy_setting_id_flags(p)       ((p) >> 24)
#define ngx_spdy_setting_id(p)       ((p) & 0x00ffffff)
...
#define NGX_SPDY_SETTINGS_INIT_WINDOW_SIZE 7
...
static ngx_http_spdy_request_header_t ngx_http_spdy_request_headers[] = {
    { 0, 7, ":method", ngx_http_spdy_parse_method },
    { 0, 7, ":scheme", ngx_http_spdy_parse_scheme },
    { 0, 5, ":path", ngx_http_spdy_parse_path },
    { 0, 8, ":version", ngx_http_spdy_parse_version },
    { 0, 5, ":host", ngx_http_spdy_parse_host },
};
...
void
ngx_http_spdy_v3_init(ngx_event_t *rev)
{
    int                          rc;
    ngx_connection_t            *c;
    ...
    u_char *pos, u_char *end)
{
    ...
    if (*(uint32_t *) pos == htonl(ngx_spdy_v3_ctl_frame_head(NGX_SPDY_SETTINGS)))
    {
        ...
        sc->need_settings = (sc->init_recv_window_size
                             == NGX_SPDY_INIT_RECV_WINDOW_SIZE) ? 0 : 1;
...
static u_char *
ngx_http_spdy_state_window_update(ngx_http_spdy_connection_t *sc,
    u_char *pos, u_char *end)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, sc->connection->log, 0,
                   "spdy WINDOW_UPDATE sid:%ui delta_window_size:%ui",
                   sid, delta_window_size);
    ...
    if (sscf->flow_control) {
        ...
        if (stream->send_window_size < 0) {
            if (ngx_http_spdy_send_rst_stream(sc, sid, NGX_SPDY_FLOW_CONTROL_ERROR,
                                              stream->priority)
                != NGX_OK)
            {
                return ngx_http_spdy_state_internal_error(sc);
...
ngx_http_spdy_send_settings(ngx_http_spdy_connection_t *sc)
{
    ...
    p = ngx_spdy_frame_aligned_write_uint32(p,
                                            NGX_SPDY_SETTINGS_FLAG_PERSIST << 24
    ...
    p = ngx_spdy_frame_aligned_write_uint32(p,
                                            NGX_SPDY_SETTINGS_FLAG_PERSIST << 24
                                            | NGX_SPDY_SETTINGS_INIT_WINDOW_SIZE);
...
static ngx_int_t
ngx_http_spdy_send_window_update(ngx_http_spdy_connection_t *sc,
                                 ngx_http_spdy_stream_t *stream,
                                 ngx_uint_t delta_window_size)
{
    u_char                     *p;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, sc->connection->log, 0,
                   "spdy create WINDOW_UPDATE frame");
...
void
ngx_http_spdy_v3_close_stream(ngx_http_spdy_stream_t *stream, ngx_int_t rc)
{
    ngx_event_t                  *ev;
    ngx_connection_t             *fc;
