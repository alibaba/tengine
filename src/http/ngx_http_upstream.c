...
ngx_http_multi_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http multi upstream send response: %p, %p", r, u);
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (u->multi) {
        if (rc == NGX_AGAIN) { //first real connect
            ...
            ngx_log_error(NGX_LOG_INFO, c->log, 0, "multi: connect new to backend %p", c);
