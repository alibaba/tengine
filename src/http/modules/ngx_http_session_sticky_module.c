    ...
    void *child)
{
    ...
    ngx_http_ss_loc_conf_t  *conf = parent;
        ...
        if (ngx_strncmp(value[i].data,
            ...
            if (ngx_strncmp(value[i].data, "indirect", value[i].len) == 0) {
                ss_srv->flag = NGX_HTTP_SESSION_STICKY_INDIRECT
                             | NGX_HTTP_SESSION_STICKY_INSERT;
            } else {
                return NGX_CONF_ERROR;
            }
        }
...
ngx_http_session_sticky_header(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        if (ngx_strncmp(value[i].data,
            ...
            if (ngx_strncmp(value[i].data + add, "http://", 7) == 0) {
                u.default_port = 80;
                add += 7;
            } else if (ngx_strncmp(value[i].data + add, "https://", 8) ==0) {
#if (NGX_HTTP_SSL)
                u.default_port = 443;
                add += 8;
#else
            ...
            if (ss_lcf->uscf == NULL) {
                return NGX_CONF_ERROR;
                             ...
                             "switch",
                             sizeof("switch") - 1) == 0) {
            value[i].data = value[i].data + sizeof("switch=") - 1;
            value[i].len = value[i].len - sizeof("switch=") + 1;
...
ngx_http_session_sticky_header_handler(ngx_http_request_t *r)
{
    ...
    if (ss_lcf->uscf == NGX_CONF_UNSET_PTR) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0,
                      "session sticky needs to configure session_sticky_header");
        return NGX_ERROR;
    }
    ...
    ngx_log_debug1(NGX_LOC_DEBUG_HTTP, r->connection->log, 0,
                   "session sticky cookie:%V", &cookie[i]->value);
    st = p;
