...
static void ngx_pipe_do_rollback(ngx_cycle_t *cycle,
...
void ngx_increase_pipe_generation(void) { ngx_pipe_generation++; }
...
ngx_int_t ngx_open_pipes(ngx_cycle_t *cycle) {
  ...
  for (i = 0; i < ngx_last_pipe; i++) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_CORE, cycle->log, 0,
                   "pipe: %ui(%d, %d) \"%s\"", i, ngx_pipes[i].pfd[0],
                   ngx_pipes[i].pfd[1], ngx_pipes[i].cmd);

    if (stat == NGX_ERROR) {
      ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                    "open pipe \"%s\" failed", ngx_pipes[i].cmd);
      return NGX_ERROR;
    }

    if (fcntl(ngx_pipes[i].open_fd->fd, F_SETFD, FD_CLOEXEC) == -1) {
      ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                    "fcntl(FD_CLOEXEC) \"%s\" failed", ngx_pipes[i].cmd);
      return NGX_ERROR;
    }

    if (ngx_nonblocking(ngx_pipes[i].open_fd->fd) == -1) {
      ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                    "nonblock \"%s\" failed", ngx_pipes[i].cmd);
      return NGX_ERROR;
    }
  ...
  for (i = 0; i < ngx_last_pipe; i++) {
    ...
    if (ngx_pipes[i].pid == pid) {
      ...
      if (close(ngx_pipes[i].open_fd->fd) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_EMERG, log, ngx_errno, "close \"%s\" failed",
                      ngx_pipes[i].cmd);
      }
      ...
      ngx_pipes[i].backup.len = ngx_pipe_dev_null_file.len;
        ...
        ngx_pipes[i].open_fd->fd =
            ngx_open_file(ngx_pipes[i].backup.data, NGX_FILE_APPEND,
...
#ifdef T_PIPES_OUTPUT_ON_BROKEN
        if (ngx_pipes[i].backup.len > 0 && ngx_pipes[i].backup.data != NULL) {
          if (chown((const char *)ngx_pipes[i].backup.data, ngx_pipes[i].user,
                    -1) == -1) {
            ngx_log_error(NGX_LOG_EMERG, log, ngx_errno,
                          "chown() \"%s\" failed", ngx_pipes[i].backup.data);
          }
        }
#endif
  ...
  if ((pid = fork()) < 0) {
    ...
    if (op->open_fd->fd != NGX_INVALID_FILE) {
      if (close(op->open_fd->fd) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                      "close \"%s\" failed", op->open_fd->name.data);
      }
    }
...
static void ngx_pipe_create_subdirs(char *filename, ngx_cycle_t *cycle) {
  ...
  for (p = filename; (p = strchr(p, '/')); p++) {
    ...
    if (ngx_file_info(dirname, &stat_buf) < 0) {
      if (errno != ENOENT) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "stat [%s] failed",
                      dirname);
        exit(2);

      } else {
        if ((ngx_create_dir(dirname, NGX_PIPE_DIR_ACCESS) < 0) &&
            (errno != EEXIST)) {
          ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
...
static void ngx_pipe_create_suitpath(ngx_cycle_t *cycle,
                                     ngx_pipe_rollback_conf_t *rbcf) {
  char realpath[256];
  struct tm *tm_now;
  int result;
  ...
  tm_now = localtime(&t);
  if (tm_now == NULL) {
    ngx_log_error(NGX_LOG_EMERG, cycle->log, 0, "get now time failed");
    return;
  }
  ...
  if (0 == result) {
    ngx_log_error(NGX_LOG_INFO, cycle->log, 0, "unlink [%s] success", realpath);
  } else {
    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "unlink [%s] failed",
                  realpath);
  }

  result = symlink(rbcf->logname, realpath);
  if (0 == result) {
    ngx_log_error(NGX_LOG_INFO, cycle->log, 0, "symlink [%s] success",
                  realpath);
  } else {
    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "symlink [%s] failed",
                  realpath);
  }
...
static time_t ngx_pipe_get_now_sec() {
  ngx_time_update();
...
static void ngx_pipe_log(ngx_cycle_t *cycle, ngx_open_pipe_t *op) {
  ...
  for (;;) {
    ...
    if (NULL != rbcf.suitpath) {
      if (rbcf.time_now / one_day > rbcf.last_suit_time / one_day) {
    ...
    if (log_fd >= 0 && rbcf.log_max_size > 0 &&
      ...
      log_fd = NGX_INVALID_FILE;
      ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                    "need check rollback size [%s] [%d]", rbcf.logname,
                    rbcf.log_size);
      ngx_pipe_do_rollback(cycle, &rbcf);
    }
    // 重写文件名
    struct tm *last = localtime(&rbcf.last_open_time);
    time_t now_unix = rbcf.time_now;
    struct tm *now = localtime(&now_unix);
    // 如果当前年小于最后一次年不处理
    if (now->tm_year < last->tm_year) {
      // 如果当前月小于最后一次月不处理
    } else if (now->tm_mon < last->tm_mon) {
    } else if (now->tm_mday > last->tm_mday) {
      if (log_fd >= 0) {
        ngx_close_file(log_fd);
      }
      log_fd = NGX_INVALID_FILE;
    ...
    if (log_fd < 0) {
      ...
      if (rbcf.interval > 0 && rbcf.last_open_time == 0 &&
          ((rbcf.time_now - rbcf.adjust_time_raw) / rbcf.interval <
           rbcf.time_now / rbcf.interval)) { /*just check
when time is after interval and befor adjust_time_raw, fix when no backup file,
do rollback
*/
        // need check last rollback time, because other process may do rollback
        // after when it adjust time more than this process
        ngx_pipe_get_last_rollback_time(&rbcf);
      } else {
        rbcf.last_open_time = ngx_pipe_get_now_sec();
      }
      char realpath[100];
      strftime(realpath, sizeof(realpath), rbcf.logname, now);
      log_fd = ngx_open_file(realpath, NGX_FILE_APPEND, NGX_FILE_CREATE_OR_OPEN,
                             NGX_PIPE_FILE_ACCESS);
      if (log_fd < 0) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "open [%s] failed",
                      rbcf.logname);
        return;
      }
...
ngx_int_t ngx_pipe_rollback_parse_args(ngx_cycle_t *cycle, ngx_open_pipe_t *op,
                                       ngx_pipe_rollback_conf_t *rbcf) {
  u_char **argv;
  ngx_uint_t i;
  ngx_int_t j;
  ...
  if (ngx_conf_full_name(cycle, &filename, 0) != NGX_OK) {
    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "get fullname failed");
    return NGX_ERROR;
  }
  ...
  for (i = 2; i < op->argv->nelts; i++) {
    ...
    if (ngx_strncmp((u_char *)"interval=", argv[i], 9) == 0) {
      ...
      rbcf->backup_num = ngx_atoi(argv[i] + 7, ngx_strlen((char *)argv[i]) - 7);
  ...
  for (j = 0; j < rbcf->backup_num; j++) {
    ...
    ngx_snprintf((u_char *)rbcf->backup[j], len, "%s.%i%Z", rbcf->logname,
                 j + 1);
  }
  ...
  ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                "log rollback param: num [%i], interval %i(S), size %i(B), "
                "adjust %i/%i(S)",
                rbcf->backup_num, rbcf->interval, rbcf->log_max_size,
                rbcf->adjust_time, rbcf->adjust_time_raw);
...
void ngx_pipe_do_rollback(ngx_cycle_t *cycle, ngx_pipe_rollback_conf_t *rbcf) {
  return;
  int fd;
  struct flock lock;
  int ret;
  ...
  if (rbcf->interval >= 0) {
    if (ngx_file_info(rbcf->backup[0], &sb) == -1) {
      ...
      need_do = 1;
      ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                    "need rollback [%s]: time on [%d] [%d]", rbcf->logname,
                    sb.st_ctime, rbcf->time_now);
    ...
    ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                  "no need check rollback [%s] time: no interval",
                  rbcf->logname);
  }

  // check size
  if (rbcf->log_max_size > 0) {
    if (ngx_file_info(rbcf->logname, &sb) == 0 &&
        (sb.st_size >= rbcf->log_max_size)) {
      need_do = 1;
      ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                    "need rollback [%s]: size on [%d]", rbcf->logname,
                    sb.st_size);
    ...
    ngx_log_error(NGX_LOG_INFO, cycle->log, 0,
                  "no need check rollback [%s] size: no max size",
                  rbcf->logname);
  }
  ...
  if (need_do) {
    ...
    if (ngx_rename_file(rbcf->logname, rbcf->backup[0]) < 0) {
      ...
      ngx_log_error(NGX_LOG_WARN, cycle->log, 0, "rollback [%s] success",
                    rbcf->logname);
    }
  }
  ngx_close_file(fd);
