...
static ngx_command_t  ngx_stream_ssl_commands[] = {
    ...
    { ngx_string("enable_ntls"),
      ...
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, enable_ntls),
      NULL },
    ...
    { ngx_string("ssl_enc_certificate"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, enc_certificate),
      NULL },
    ...
    { ngx_string("ssl_enc_certificate_key"),
      ...
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, enc_certificate_key),
      NULL },
    ...
    { ngx_string("ssl_sign_certificate"),
      ...
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, sign_certificate),
      NULL },
    ...
    { ngx_string("ssl_sign_certificate_key"),
      ...
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, sign_certificate_key),
      NULL },
#endif
    ...
    { ngx_string("ssl_sni_force"),
      ...
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_srv_conf_t, sni_force),
      NULL },
#endif
...
ngx_stream_ssl_servername(ngx_ssl_conn_t *ssl_conn, int *ad, void *arg)
{
    ...
    ngx_stream_core_srv_conf_t  *cscf;
#if (T_NGX_STREAM_SNI)
    ngx_uint_t                   sni_matched;  /* Tengine: for ssl_sni_force */
#endif
...
#if (T_NGX_STREAM_SNI)
    sni_matched = 0;  /* Tengine: ssl_sni_force, no virtual server matched yet */
#endif
...
#if (T_NGX_STREAM_SNI)
    sni_matched = 1;  /* Tengine: ssl_sni_force, virtual server matched */
#endif
...
#if (T_NGX_STREAM_SNI)
    /*
     * Tengine extension: with "ssl_sni_force" on, a client-supplied SNI that
     * does not match any virtual server is rejected with a fatal alert.
     */

    if (!sni_matched && sscf->sni_force) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                      "SSL SNI does not match any server name, rejected");
        *ad = SSL_AD_UNRECOGNIZED_NAME;
        return SSL_TLSEXT_ERR_ALERT_FATAL;
    }
#endif
...
{
    ngx_stream_ssl_srv_conf_t  *scf = conf;
...
#if (T_NGX_HAVE_DTLS)
            /* Tengine: DTLS protocol consistency for this ssl listen */
            if (addr[a].opt.type == SOCK_DGRAM) {
...
#if (T_NGX_SSL_NTLS)
                || sscf->enc_certificate.len
                || sscf->sign_certificate.len
#endif
               )
            {
                continue;
