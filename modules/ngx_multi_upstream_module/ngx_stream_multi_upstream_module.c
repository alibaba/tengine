    ...
    ngx_uint_t state)
{
    ...
    ngx_log_error(NGX_LOG_INFO, pc->log, 0, "multi: free request c: %p, r: %p end", pc->connection, session);
    ...
    if (q) {
        ...
        ngx_log_error(NGX_LOG_WARN, pc->connection->log,
                      0, "multi: free stream session not found %p, %p", session, pc->connection);
    }
